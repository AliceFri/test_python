"""
该刷题团队的管理模式可以用一棵树表示: 团队只有一个负责人,编号为1。除了该负责人外，每个人有且仅有一个领导（负责人没有领导）；
...
leadership是大小为(N - 1) * 2
的二维数组，其中每个元素[a, b]
代表b是a的下属；
operations是一个长度为Q的二维数组，代表以时间排序的操作，格式如下：
operations[i][0] = 1: 代表第一种操作，operations[i][1]
代表成员的编号，operations[i][2]
代表LeetCoin的数量；
operations[i][0] = 2: 代表第二种操作，operations[i][1]
代表成员的编号，operations[i][2]
代表LeetCoin的数量；
operations[i][0] = 3: 代表第三种操作，operations[i][1]
代表成员的编号；
输出：
...
class Solution:
    def bonus(self, n: int, leadership: List[List[int]], operations: List[List[int]]) -> List[int]:
        leader = collections.defaultdict(list)
        e7 = 10 ** 9 + 7
        for l in leadership:
            leader[l[0]].append(l[1])

        # dfs 序
        l = []
        fw = collections.defaultdict(list)  # 每个点掌管的范围
        ...
        dfs(1)
        dIndex = {i: ind + 1 for ind, i in enumerate(l)}
        ...
        def pushdown(rt, ln, rn):
            if adds[rt]:
                nums[rt << 1] = (nums[rt << 1] + ln * adds[rt]) % e7
                nums[rt << 1 | 1] = (nums[rt << 1 | 1] + rn * adds[rt]) % e7
                adds[rt << 1] = (adds[rt << 1] + adds[rt]) % e7
                adds[rt << 1 | 1] = (adds[rt << 1 | 1] + adds[rt]) % e7
                adds[rt] = 0

        def add(L, R, C, l, r, rt):
            if l >= L and r <= R:
                nums[rt] = (nums[rt] + C * (r - l + 1)) % e7
                adds[rt] = (adds[rt] + C) % e7
                return
            mid = l + ((r - l) >> 1)
            ...
            nums[rt] = (nums[rt << 1] + nums[rt << 1 | 1]) % e7
        ...
        def query(L, R, l, r, rt):
            ...
            icnt = 0
            if l <= R and mid >= L:
                icnt += query(L, R, l, mid, rt << 1)
            if mid + 1 <= R and r >= L:
                icnt += query(L, R, mid + 1, r, rt << 1 | 1)
            return icnt % e7
        ...
        for op in operations:
            if op[0] == 1:
                ind = dIndex[op[1]]
                add(ind, ind, op[2], 1, n, 1)
            elif op[0] == 2:
                inds = fw[op[1]]
                add(inds[0], inds[1], op[2], 1, n, 1)
            elif op[0] == 3:
                inds = fw[op[1]]
                res.append(query(inds[0], inds[1], 1, n, 1))
        return res
